...
# Simplest example EVER to show how the class step_by_step_brian_sim works!
# Remember: the spike neuron model used here cannot receive spikes during the simulation step it is spiking!
def simulation_example(brian_clock, brian):
    ...
    Output_layer = brian.NeuronGroup(Number_of_input_neurons,model='v:1', reset=0, threshold=10)
...
# Testing the step-by-step simple "simulation"
# Starting the simulation object!

# Initializing the simulation...
step_size = 0.5
s = step_by_step_brian_sim(simulation_example, step_size)
...
for i in range(10):
    input_spikes = [1,2,3,12]
    # input_spikes = None
    r,current_t,r_spikes,OutputSpks,t = s.run_step(input_spikes) # Sends the spikes and runs to the second yield 
                                            #(where the generator returns the result of the simulation)

    print (r,current_t,r_spikes)
    print (OutputSpks,t)